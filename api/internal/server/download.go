...
// Ele delega ao mesmo caminho do downloader multiplataforma: manter uma segunda
// implementação de download aqui significaria corrigir cada bug duas vezes.
//
// O contrato de entrada continua o mesmo ({type, url}), e a validação de URL
// segue restrita ao YouTube, como sempre foi para esta rota.
func (s *Server) createDownload(ctx *gin.Context) {
	...
	// Sem format_id: o provider escolhe o melhor formato do tipo pedido, que é
	// exatamente o que esta rota sempre fez.
	s.criarDownloadMedia(ctx, criarDownloadMediaRequest{URL: req.URL, Kind: kind})
}
...
func (s *Server) getDailyDownloads(ctx *gin.Context) {
	...
	// O super admin não tem teto; o contador precisa dizer isso, ou a tela
	// bloqueia o campo de URL assim que o limite nominal é atingido.
